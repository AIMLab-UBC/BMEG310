...
```{r}
setwd("C:\\Personal\\BMEG310 2023\\BMEG310_2022\\Diff expression")
# Import metadata
...
## Volcano plot
A volcano plot is a type of scatter plot that represents the differential expression of features (e.g., genes). In this plot, the log-transformed adjusted p-values are displayed on the y-axis, while the log2 fold change values are plotted on the x-axis. The code in the chunk below includes two functions: one for preparing the data for the volcano plot and another for plotting the volcano plot.
...
Now we can call the above functions. By adjusting the P-adj and log2-fold change thresholds, you can identify different significantly upregulated and downregulated genes. This serves as a visualization to help interpret the DESeq results.
...
To get the **ALL** significant genes
...
Alternatively, you can select a subset of all significant genes, such as the top 20 upregulated and downregulated genes. First, we filter the significant genes. Then, we select the 20 genes with the largest positive log2 fold change.
...
We initially generated PCA and heatmap plots using the raw count data. However, for proper visualization, the data should be normalized and transformed. Below are the heatmap and PCA plots for all the data (including both significant and insignificant genes), using the normalized and transformed data.
...
Instead of using all the data, we can filter for significant genes or the top upregulated and downregulated genes. By using the normalized counts and the indices of the significant or top genes, we can plot the PCA and heatmap for a more focused visualization. This approach highlights the key features driving the biological differences in the dataset.

```{r}
annot_col = data.frame(colData$condition)
row.names(annot_col) <- rownames(colData)

sampleMatrix <- assay(vsd)[genes.significant,]
# sampleMatrix <- assay(vsd)[genes.top,]

rownames(sampleMatrix) = rownames(countData[genes.significant,])
# rownames(sampleMatrix) = rownames(countData[genes.top,])
